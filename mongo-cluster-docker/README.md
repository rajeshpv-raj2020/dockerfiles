...
* tested with docker on mac mojave
  
```bash
cd $MY_WORKSPACE/docker/mongo-cluster-docker
# start one-time in detach mode
docker pull mongo:4.0.1
./dc.sh up  -d --no-recreate --no-color

# many times as you need
./dc.sh ps
# list containers, you notices router is at port 30001
./dc.sh stop
./dc.sh start
```

* connect mongo client at mongodb://localhost:30001
* I tested with mongo 4.0 client from host on mac as follows 
```bash
mongo --host localhost --port 30001

db.sample.find(); # you find none
load("queries/init.js")
db.sample.find(); #should see data
```
