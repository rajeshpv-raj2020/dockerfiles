...
public class RequestThread extends Thread {
    ...
    private static void sendError(BufferedOutputStream out, int code, String message) throws IOException {
        message = message + "<hr>" + BrowseHere.VERSION;
    ...
    public void run() {
        ...
        try {
            ...
            */if (!file.exists()) {
                ...
                out.write(("</p><hr><p>" + BrowseHere.VERSION + "</p></body><html>").getBytes());
            }
            else {
                reader = new BufferedInputStream(new FileInputStream(file));
            
                String contentType = (String)BrowseHere.MIME_TYPES.get(getExtension(file));
                if (contentType == null) {
    ...
    private static  void sortyByModified(File[] files){
   ...
   private static  void sortyByName(File[] files){
        Arrays.sort(files, new Comparator<File>(){
        public int compare(File f1, File f2)
        {
